...
uint16_t LcdRead16(void)
{
  ...
  LCD_READ_CLK;
  if(GPIOX_IDR(LCD_MISO)) d16 |= 0x800;
  LCD_READ_CLK;
  if(GPIOX_IDR(LCD_MISO)) d16 |= 0x400;
  LCD_READ_CLK;
  if(GPIOX_IDR(LCD_MISO)) d16 |= 0x200;
  LCD_READ_CLK;
  if(GPIOX_IDR(LCD_MISO)) d16 |= 0x100;
  LCD_READ_CLK;
  if(GPIOX_IDR(LCD_MISO)) d16 |= 0x80;
  LCD_READ_CLK;
  if(GPIOX_IDR(LCD_MISO)) d16 |= 0x40;
  LCD_READ_CLK;
  if(GPIOX_IDR(LCD_MISO)) d16 |= 0x20;
  LCD_READ_CLK;
  if(GPIOX_IDR(LCD_MISO)) d16 |= 0x10;
  LCD_READ_CLK;
  if(GPIOX_IDR(LCD_MISO)) d16 |= 0x8;
  LCD_READ_CLK;
  if(GPIOX_IDR(LCD_MISO)) d16 |= 0x4;
  LCD_READ_CLK;
  if(GPIOX_IDR(LCD_MISO)) d16 |= 0x2;
  LCD_READ_CLK;
  if(GPIOX_IDR(LCD_MISO)) d16 |= 0x1;
  GPIOX_SET(LCD_SCK);
  return d16;
}
...
#elif LCD_SPI == 2
#define SPIX                  SPI2
#define LCD_SPI_RCC_EN        RCC->APB1ENR1 |= RCC_APB1ENR1_SPI2EN
#elif LCD_SPI == 3
#define SPIX                  SPI3
#define LCD_SPI_RCC_EN        RCC->APB1ENR1 |= RCC_APB1ENR1_SPI3EN
#endif

#define LcdSpiMode8()         SPIX->CR2 &= ~(SPI_CR2_DS_3)
#define LcdSpiMode16()        SPIX->CR2 |= (SPI_CR2_DS_3)
