...
// LCD_D0..LCD_D15 lábak azonos porton ás növekvö sorrendben vannak vannak
#define LCD_AUTOOPT
#endif // D0..D15 portláb folytonosság ?
#endif // D0..D15 port azonosság ?
...
#define LCD_DATA16_WRITE(dt) {    \
  lcd_data16 = dt;                \
  LCD_WRITE(lcd_data16);          \
  GPIOX_ODR(LCD_WR) = 0;          \
  LCD_IO_Delay(LCD_IO_RW_DELAY);  \
  GPIOX_ODR(LCD_WR) = 1;          }

#define LCD_DATA16_READ(dt) {     \
  GPIOX_ODR(LCD_RD) = 0;          \
  LCD_IO_Delay(LCD_IO_RW_DELAY);  \
  LCD_READ(lcd_data16);           \
  dt = lcd_data16;                \
